...
final class NeuralNet {
    ...
    init(structure: Structure, randomizeLastLayer: Bool = false, weights: [[Float]]? = nil) throws {
        ...
        if let weights = weights {
            ...
            randomizeWeights(randomizeLastLayer)
        }
    }
...
extension NeuralNet {
    ...
    fileprivate func randomizeWeights(_ includeLastLayer: Bool = false) {
        ...
        let range = includeLastLayer ?  1..<numLayers :  1..<(numLayers - 1)
        for layer in range{
            // Randomize weights
            for weight in 0..<cache.layerWeightCounts[layer] {
                let fanIn = layerNodeCounts[layer - 1]
                let fanOut = (layer == numLayers-1) ? 0 : layerNodeCounts[layer + 1]
                cache.layerWeights[layer][weight] = randomWeight(fanIn: fanIn , fanOut: fanOut)
            }
        }
    }
